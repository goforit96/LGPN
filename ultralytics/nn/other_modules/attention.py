...
__all__ = ['EMA', 'SimAM', 'SpatialGroupEnhance', 'BiLevelRoutingAttention', 'BiLevelRoutingAttention_nchw',
           ...
           'TransNeXt_AggregatedAttention','ECAAttention','SPPF_LSKA','ShuffleAttention','ELA','CAA','CPCA','MLCA']
...
def get_relative_position_cpb(query_size, key_size, pretrain_size):
    pass


def AggregatedAttention(dim, input_resolution, num_heads, window_size, qkv_bias, attn_drop, proj_drop, sr_ratio):
    pass
...
class ELA(nn.Module):
    ...
    def forward(self, x):
        ...
        x_h = self.Attention(self.conv1x1(self.pool_h(x).reshape((b, c, h))).reshape((b, c, h, 1)))
...
class CAA(nn.Module):
    def __init__(self, ch, h_kernel_size=11, v_kernel_size=11) -> None:
        ...
        self.avg_pool = nn.AvgPool2d(7, 1, 3)  #池化核为7, 步长为1, 填充为3, 对输入特征图进行下采样，减少特征图的尺寸。
        self.conv1 = Conv(ch, ch)              # 输入和输出通道数都为 ch
        self.h_conv = nn.Conv2d(ch, ch, (1, h_kernel_size), 1, (0, h_kernel_size // 2), 1, ch)   #水平方向的一维卷积  卷积核在水平方向上延展   其中groups为ch 表示每个通道的卷积操作是独立进行的，叫做深度可分离卷积
        self.v_conv = nn.Conv2d(ch, ch, (v_kernel_size, 1), 1, (v_kernel_size // 2, 0), 1, ch)   #垂直方向的一维卷积  卷积核在垂直方向上延展
        self.conv2 = Conv(ch, ch)     #输入和输出通道数也是 ch
        self.act = nn.Sigmoid()       #一个 Sigmoid 激活函数，用来将网络的输出映射到 [0, 1] 的范围内，通常用于生成注意力权重
...
################## 添加注意力机制CPCAttention ###############################
    ...
    class CPCA(nn.Module):
        def __init__(self, channels, channelAttention_reduce=4):
            ...
            self.ca = CPCA_ChannelAttention(input_channels=channels,
                                            internal_neurons=channels // channelAttention_reduce)
            self.dconv5_5 = nn.Conv2d(channels, channels, kernel_size=5, padding=2, groups=channels)
            self.dconv1_7 = nn.Conv2d(channels, channels, kernel_size=(1, 7), padding=(0, 3), groups=channels)
            self.dconv7_1 = nn.Conv2d(channels, channels, kernel_size=(7, 1), padding=(3, 0), groups=channels)
            self.dconv1_11 = nn.Conv2d(channels, channels, kernel_size=(1, 11), padding=(0, 5), groups=channels)
        ...
        class MLCA(nn.Module):
            ...
            def forward(self, x):
                ...
                y_local_transpose = y_local.reshape(b, self.local_size * self.local_size, c).transpose(-1, -2).view(b,
                                                                                                                    c,
                                                                                                                    self.local_size,
                                                                                                                    self.local_size)
                y_global_transpose = y_global.view(b, -1).transpose(-1, -2).unsqueeze(-1)
