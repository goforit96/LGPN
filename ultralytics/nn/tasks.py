...
from ultralytics.nn.other_modules import Detect_DyHead

from ultralytics.nn.modules.Biformer import BiLevelRoutingAttention, Attention, AttentionLePE
from ultralytics.nn.modules.SlimNeck import LFEM, VoVGSCSPC, GSConv #导入SlimNeck包
from ultralytics.nn.modules.SlimNeck import ContextAggregation,space_to_depth

from ultralytics.nn.other_modules import LSKA ,ECAAttention,CARAFE,DepthWiseConv,SPPF_LSKA,ShuffleAttention,ELA,CAA,CPCA,MLCA,Ghost_HGBlock
from ultralytics.nn.modules.block import (ConvNextBlock ,ODConv_3rd,RepBlock,Concat_BiFPN,CoordAtt)
from ultralytics.nn.modules import (AIFI, C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x,
                                    Classify, Concat, Conv, Conv2, ConvTranspose, Detect, DWConv, DWConvTranspose2d,
                                    Focus, GhostBottleneck, GhostConv, HGBlock, HGStem, Pose, RepC3, RepConv,
                                    ResNetLayer, RTDETRDecoder, Segment,DCFE,CBAM,ChannelAttention,EfficientAttention,GAM_Attention,MCALayer,RepBlock,BiFusion,PolarizedSelfAttention,SelfAttention,
                                    BiFPN_Add3,BiFPN_Add2,TripletAttention,GA)
from ultralytics.nn.modules.block import SimFusion_4in, AdvPoolFusion, SimFusion_3in, InjectionMultiSum_Auto_pool, \
    TopBasicLayer, PyramidPoolAgg, IFM
from ultralytics.nn.other_modules.head import DetectAux
...
def parse_model(d, ch, verbose=True,warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3,DCFE, GSConv, LFEM, VoVGSCSPC,ContextAggregation,ConvNextBlock,ODConv_3rd,RepBlock,
                 EfficientAttention,CoordAtt,RepBlock,DepthWiseConv,SelfAttention,GA):
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3,DCFE,RepBlock,
                     GA,EfficientAttention,CoordAtt,RepBlock,DepthWiseConv,PolarizedSelfAttention,SelfAttention,SPPF_LSKA,
                      ):
        ...
        elif m in (HGStem, HGBlock, Ghost_HGBlock):
            c1, cm, c2 = ch[f], args[0], args[1]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
        ...
        elif m in (Detect, Segment, Pose,Detect_DyHead):
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
        elif m is RTDETRDecoder:  # special case, channels arg must be passed in index 1
            args.insert(1, [ch[x] for x in f])
        elif m in{CBAM,ChannelAttention,BiLevelRoutingAttention, Attention, AttentionLePE,CoordAtt,EfficientAttention}:
            c1, c2 = ch[f], args[0]
            if c2 !=nc:
                c2 = make_divisible(min(c2, max_channels)*width, 8)
                args=[c1,*args[1:]]
