...
from ..other_modules.attention import SEAttention ,ELA

__all__ = ['GSConv', 'LFEM', 'VoVGSCSPC','space_to_depth' ]
...
class LFEM(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        #self.Attention = SEAttention(c_)
        self.Attention = ELA(c_)

    def forward(self, x):
        x1 = self.gsb(self.cv1(x))
       # x1 =self.Attention(self.gsb(self.cv1(x)))
        y = self.cv2(x)
        #y =self.Attention(self.cv2(x))
        #return self.cv3(torch.cat((y, x1), dim=1))

        return self.cv3(torch.cat((y, x1), dim=1))
        #return self.Attention(self.cv3(torch.cat((y, x1), dim=1)))
